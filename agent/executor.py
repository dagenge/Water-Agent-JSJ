...
- **布吉河 (bjh)**：5站，广东流域，含流量数据
- **棠荆 (tj)**：7站，广东流域，含流量数据
- **尖山 (js)**：18站，广东流域，含流量数据
- **河子口 (hzk)**：7站，广东流域，含流量数据
- **白盆珠水库 (bpz)**：7站，广东流域，含流量数据
...
2. 广东流域（bjh/tj/js/hzk/bpz）的事件查询：
   ...
   - 统计分析（含流量）：query_guangdong_statistics(basin_id, event_code)
3. 定曲河（dqh）事件查询：query_dqh_events / query_dqh_rainfall / query_dqh_statistics
